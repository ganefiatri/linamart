            ...
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="d-flex flex-wrap justify-content-center mb-3">
                    <div class="d-flex align-items-center">
                        <h4 class="mb-2">{{ __('Incoming Assignments') }}</h4>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-8 col-md-8 col-sm-8 col-12 mb-3">
            ...
            <div class="col-lg-4 col-md-4 col-sm-4 col-12 mb-3">
                ...
                <!-- endsearch -->
                <div class="card">
                    <div class="card-header color-dark fw-500">{{ __('Delivered Order') }}</div>
                    <div class="card-body p-2">
                        <div class="userDatatable global-shadow border-0 bg-white w-100">
                            <div class="table-responsive">
                                <table class="table mb-2 table-striped">
                                    ...
                                    @foreach ($delivereds as $delivered)
                                        <tr>
                                            <td class="text-center">
                                                {{ $i + (($delivereds->currentPage()-1) * $delivereds->perPage()) }}
                                            </td>
                                            <td>
                                                <a href="{{ route('driver.assignments.show', $delivered->id) }}">{{ $delivered->getInvoiceNumber() }}</a> - {{ $delivered->buyer_name }}
                                            </td>
                                            <td>
                                                {{ !empty($delivered_at = $delivered->getDeliveredAt()) ? date("d F Y H:s", strtotime($delivered_at)) : '-' }}
                                            </td>
                                        </tr>
                                        @php ++$i @endphp
                                    @endforeach
                                ...
                                {{ $delivereds->appends(request()->input())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
...
@section('js')
    ...
    {
        if (confirm("{{ __('Are you sure you want to mark this order as delivered?') }}")) {
            $(dt).parent().submit();
        }
        return false;
    }
